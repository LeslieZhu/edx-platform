...
class SequenceModule(SequenceFields, ProctoringFields, XModule):
    ...
    def student_view(self, context):
        ...
        banner_text = None
        special_html_view = self._special_exam_student_view() or self._hidden_content_student_view()
    ...
    def _hidden_content_student_view(self):
        """
        Checks whether the content of this sequential is hidden from users.
        ...
        if not self._can_view_content():
            ...
            ).format(subsection_format=subsection_format)  # pylint: disable=no-member

            hidden_content_html = self.system.render_template(
                'hidden_content.html',
                {'subsection_format': subsection_format}  # pylint: disable=no-member
            )
